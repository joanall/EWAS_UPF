...
# Title: Enrichment to use with EASIER package
# Date: 03/01/2023

# NOTE: RUN IN INTERACTIVE SESSION to avoid following error .. Error in FUN(X[[i]], ...) : org.Hs.eg.db not installed

# Note: In Easier package there are 2 ways of runing enrichment:
# 1) give list og Cpgs as input
# 2) give gwama output -> it also uses statistic as bf and fdr to do some part of the enrichment

# Since we want to include those suggestive CpGs in all models (not only in main)
# We manually create a .txt file with the cpg ids to perform the ernichemnt on 
...
FilesToEnrich <- c('new_version_14_02_2024/results/GWAMA_Results/Meta_Model_3CT_Filtr/Meta_Model_3CT_Fixed_Modif.out')
#FilesToEnrich <- c('results/Enrichment/All_Enrichment_1E05.txt')

# Values for adjustment
BN <-  FALSE    # Use Bonferroni ?
FDR <- NA     # significance level for adjustment, if NA FDR is not used
pvalue <- 1E-04 # significance level for p-value, if NA p-value is not used
...
results_folder <- 'results/QC_Results'
results_gwama <- 'results/'
results_enrich <- 'new_version_14_02_2024/results/Enrichment/All_Enrichment_1E04'
...
# Perform eQTM enrichment -> doesn't work
bEQTM <- TRUE
