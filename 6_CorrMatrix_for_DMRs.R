...
# Date: 13/01/2023
# Description:  Calculates the correlations between each CpG site and the next 20 CpG 
...
filename <- "results/EWAS_RESULTS/HELIX/rlm.result.model.3.CT.B.rds" 

methylation_directory <- "db/final_bVals.RDS"

#  Define output/results directory to save output files
output_folder <- 'new_version_14_02_2024/results/ANALYZE_DMRs/MODEL_3/'
...
load(methylation_directory) # 1138 386518
...
ifelse(all(annotation$name==rownames(methylation)), 
       "meth and annotation data successfully matched :) ","Data not matched :(")
...
# Save matrix
write_rds(pre, file = paste0(output_folder, cohort, "CorrMatrix_",
                             basename(filename)))
